...
const AddEducation = () => {
  const { categories, setCategories, newCategory, setNewCategory, newSkill, setNewSkill, 
    ...
    return (
      ...
      <h2 className="lg:text-xl sm:text-lg font-bold mb-4">Your Education</h2>
      {categories.map((category, categoryIndex) => (
          <div key={categoryIndex} className="mb-4">
              {editCategoryIndex === categoryIndex ? (
                  ...
                  <input
                      ...
                      className="border border-gray-300 lg:w-80  sm:w-fit p-2 mr-2 placeholder:text-[14px]"
          ...
          <div className="mt-4">
              <button onClick={storeEducationInFirebase} className="px-4 py-2 text-sm bg-green-500 text-white rounded p-2  text-white rounded transistion-transform hover:transform  hover:scale-90">{updateNow}</button>
          </div>
      </div>
  );
};
